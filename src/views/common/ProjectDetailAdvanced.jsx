...
const ProjectDetailAdvanced = ({ project, onBack, onNavigate }) => {
  const { user } = useAuth();
  ...
  const tabLabels = ['Información General', 'Diagrama de Gantt', 'Métricas', 'Tareas', 'Participantes', 'Archivos'];
  ...
  const loadProjectData = async () => {
    try {
      ...
      // Convertir tareas para Gantt
      const ganttData = convertTasksToGantt(projectTasks);
  ...
  const convertTasksToGantt = (projectTasks) => {
    return projectTasks.map(task => ({
      start: new Date(task.start_date || task.created_at),
      end: new Date(task.due_date || task.end_date || addDays(new Date(task.created_at), 7)),
      name: task.title,
      id: task.id.toString(),
      type: 'task',
      ...
      styles: {
        progressColor: getTaskColor(task.priority),
        progressSelectedColor: getTaskColor(task.priority),
        backgroundColor: `${getTaskColor(task.priority)}40`,
        backgroundSelectedColor: `${getTaskColor(task.priority)}60`
      }
    }));
  };
  ...
  const handleTaskChange = (task) => {
    if (!canEditTask(tasks.find(t => t.id === parseInt(task.id)))) return;

    const updatedTask = {
      ...tasks.find(t => t.id === parseInt(task.id)),
      start_date: task.start.toISOString(),
      due_date: task.end.toISOString()
    };

    try {
      dataService.update('tasks', parseInt(task.id), updatedTask);
                  ...
                  <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
                    ...
                    <Box sx={{ display: 'flex', gap: 1 }}>
                      <IconButton 
                        ...
                        sx={{ color: viewMode === ViewMode.Day ? GREEN : '#666' }}
                      >
                        <CalendarViewDayIcon />
                      </IconButton>
                      <IconButton 
                        onClick={() => handleViewModeChange(ViewMode.Week)}
                        sx={{ color: viewMode === ViewMode.Week ? GREEN : '#666' }}
                      >
                        <CalendarViewWeekIcon />
                      </IconButton>
                      <IconButton 
                        onClick={() => handleViewModeChange(ViewMode.Month)}
                        sx={{ color: viewMode === ViewMode.Month ? GREEN : '#666' }}
                      >
                  ...
                  {ganttTasks.length > 0 ? (
                    <Box sx={{ height: 400, overflow: 'auto' }}>
                      <Gantt
                        ...
                        onTaskDelete={() => {}} // Implementar si es necesario
                        onProgressChange={() => {}} // Implementar si es necesario
                        onDoubleClick={(task) => {
                    ...
                    <Box sx={{ textAlign: 'center', py: 4 }}>
                      ...
                      <Typography variant="body2" sx={{ color: '#999' }}>
                        Agrega tareas al proyecto para ver el diagrama de Gantt
                      </Typography>
                    </Box>
                  )}
                </CardContent>
...
export default ProjectDetailAdvanced;
